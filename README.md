...
| [java](java) | Same scenario in Java, mixing JPA (`@Id`, `@GeneratedValue`) and Stormify (`@DbTable`, `@DbField`) annotations in one project. | `mvn compile exec:java` |
...
- Want the shortest path to a working Stormify app? → [`kotlin-jvm`](kotlin-jvm) or [`java`](java).
